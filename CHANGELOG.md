...
### Fixed
- Keep Samsung WAF's side menus and Launcher3 bottom taskbar in one child-facing lifecycle: both are hidden together on the kiosk and allowed external apps, both are restored in PIN/Settings, and a lightweight native reconciliation loop repairs either OEM surface if it starts late or is recreated.
