...
### Added
- Add an admin-controlled, boot-persistent wireless-debugging policy for Device Owner deployments. It keeps Android ADB and paired wireless debugging enabled independently of Lock Mode or the FreeKiosk process, makes trusted-host authorizations non-expiring, reports whether the legacy TCP 5555 listener is currently active, and clearly distinguishes Android's protected fixed-port provisioning from the setting FreeKiosk can enforce.
...
### Fixed
- Replace the duplicated external-app controls with one square **Close All** button. The legacy fixed return button is suppressed for external-app sessions, and on Samsung WAF the ordinary close mirror is also suppressed so only the higher accessibility-layer control is rendered; Disney+ playback transitions therefore cannot expose a second copy. The button is half the previous size (32 dp), fixed to the absolute bottom-right corner, remains reconciled for the complete child-app session, and closes every configured allowed app before returning to the kiosk grid. Admin multi-tap remains available through a tiny invisible target at the extreme corner of the same control.
- Keep the display awake against automatic timeout for the entire Lock Mode lifecycle, including while Disney+, Netflix, or another external child app owns the foreground window, without overriding a deliberate power-button press. A short press now leaves the screen off until the next short press, then returns directly to the same locked child app; only the existing optional Auto Wake setting can request an immediate wake.
- Stop the external-app monitor from relaunching an app's launcher Activity over its own playback Activity. Modern Android can return FreeKiosk's background task from the restricted `getRunningTasks()` API, which made the monitor restart Disney+ or Netflix every five seconds and immediately end video playback. Foreground resolution now uses lifecycle Usage Events first and keeps the task API only as an old-device fallback.
- Reduce the Samsung 75WAF accessibility edge guards from 40 dp to 12 dp. The guards still own the initial physical-edge touch that starts the OEM reveal gesture, but the visible top/bottom safety bands no longer crop 144 pixels from video and app content.
- Block Samsung 75WAF left/right edge swipes in addition to the top/bottom gestures. Four accessibility-layer edge guards now own every physical display edge for the complete saved kiosk lifecycle, including the app grid and lock-task recovery intervals. The WAF desktop/freeform, multi-window control-bar, and side-button firmware switches are preserved and forced off while Lock Mode is active; multi-window/finish attempts also immediately repair strict lock task and relaunch fullscreen instead of minimizing the kiosk onto the launcher.
- Block the Samsung 75WAF firmware's top-status and bottom-navigation reveal gestures while the kiosk is child-facing. The OEM `skip_swipe_bottom_top` and `skip_gesture` policy switches are preserved and continuously reconciled, while accessibility-layer edge shields prevent WAF's higher-layer SystemUI windows from receiving the initial swipe over an external app. The mandatory close and admin tap controls are mirrored in that same higher safety layer so app SurfaceViews cannot cover them. The original OEM values are restored only on an explicit kiosk exit.
- Make the custom close control mandatory and persistent for every launched child app. Overlay-owned accessibility events no longer remove it, Android Back/Home/Recents key events are consumed for the active child session, Samsung's hidden-navigation gesture policy is enforced, and an unexpected Back return immediately restores the selected app instead of stopping the overlay on the multi-app grid. Only the custom close button clears the session and returns to the kiosk home.
- Keep the PIN and authenticated administration screens in the foreground after returning from an external app. Entering the admin flow now creates an explicit native session that atomically clears the selected child app and disarms its overlay, foreground monitor, re-pin loop, watchdog recovery, and JavaScript AppState relaunch paths until the kiosk home is deliberately reopened. Accessibility events attributed to Android helper services are also ignored unless that service actually owns the foreground Activity, preventing a text-input event from injecting Back or resetting FreeKiosk's navigation stack while the administrator enters the PIN or edits Settings.
- Fail closed to the FreeKiosk home when Device Owner lock task is missing instead of reopening a stale external app. The selected child app is now process-scoped, every native relaunch re-verifies `LOCKED` state and the configured allowlist, and the return/admin overlay starts atomically only after the strict boundary is active. This removes the reboot/process-death Disney+ relaunch loop that could leave Android's top, bottom, Home, and Recents surfaces exposed.
- Remove Samsung's fullscreen desktop-window menu while Lock Mode is active and opt the kiosk out of Android 16's forced large-screen resizability. This closes an OEM escape where the header's fullscreen action crashed the desktop shell, hid the kiosk window, and exposed SystemUI even though Android still reported lock task as active.
- Keep Android lock task and all Samsung WAF side, top, and bottom system surfaces enforced on every FreeKiosk route, including PIN and authenticated kiosk administration. System chrome is now restored only by an explicit Lock Mode exit; every foreground focus also verifies and repairs Device Owner lock task.
- Keep the Samsung WAF side menus and bottom taskbar hidden on the kiosk password screen, restoring them only after successful authentication reaches Settings. Also refresh React Native's root layout constraints after WAF density transitions so returning from Settings keeps app grids centered and corner return controls tappable.
- Keep Samsung WAF's side menus and Launcher3 bottom taskbar in one child-facing lifecycle: both are hidden together on the kiosk, PIN gate, and allowed external apps, both are restored in authenticated Settings, and a lightweight native reconciliation loop repairs either OEM surface if it starts late or is recreated.
